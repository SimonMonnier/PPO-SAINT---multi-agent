...
# Version "Loup Scalpeur" — version modifiée :
#   - Reward Sharpe-like sur log-return d'equity (excess_ret + Sharpe local)
#   - Position sizing simplifié : size = position_size * risk_scale (1x / 1.8x)
...
#   - OBS = uniquement OHLC + indicateurs M1/H1 (pas de PnL ni position dans l'input)
#
#   - AJOUT : spécialisation d'agent via cfg.side :
#       side = "both"  → comportement original (BUY + SELL)
#       side = "long"  → agent spécialisé BUY (0,2,4)
#       side = "short" → agent spécialisé SELL (1,3,4)
# ======================================================================
...
class PPOConfig:
    ...
    tp_shrink: float = 0.95   # Réduit le TP de 5%
    ...
    position_size: float = 0.06      # base lot size (sera multiplié par risk_scale)
    leverage: float = 6.0
    ...
    risk_per_trade: float = 0.012    # (plus utilisé pour le sizing dynamique)
...
class BTCTradingEnvDiscrete(gym.Env):
    ...
    Actions agent (single-head + mask, 5 actions) :
      ...
      - si position == 0 :
            actions valides = dépend de cfg.side
              "both" : {0,1,2,3,4}
              "long" : {0,2,4}
              "short": {1,3,4}
      - si position != 0 :
            actions valides = {4} uniquement, la fermeture se fait
            UNIQUEMENT par SL/TP ATR.

    Observation :
        - fenêtre (lookback, N_BASE_FEATURES) des features M1/H1 normalisées
        - pas d'info d'état de trading (pas de PnL latent, ni last_real, ni pos one-hot)
    ...
    def _get_obs(self):
        ...
        obs = self.data.features[start:self.idx]  # (lookback, N_BASE_FEATURES)
    ...
    def _compute_dynamic_size(self, price: float) -> float:
        """
        Position sizing simplifié :
          - taille = position_size * risk_scale
          - plus aucune dépendance à l'ATR ou au capital.
        """
...
# --------- Masques d'actions avec spécialisation long / short ---------

def build_mask_from_pos_scalar(pos: int, device, side: str) -> torch.Tensor:
    """
    Renvoie un bool mask (N_ACTIONS,) True = action valide.

    - Si pos != 0 : uniquement HOLD (4)
    - Si pos == 0 (flat) :
        side == "both"  : toutes les actions valides (0..4)
        side == "long"  : BUY1 (0), BUY1.8 (2), HOLD (4)
        side == "short" : SELL1 (1), SELL1.8 (3), HOLD (4)
    """
    mask = torch.zeros(N_ACTIONS, dtype=torch.bool, device=device)

    # En position → seulement HOLD
    if pos != 0:
        mask[4] = True
        return mask
    ...
    if side == "both":
        ...
        mask[0] = True   # BUY 1x
        mask[2] = True   # BUY 1.8x
        mask[4] = True   # HOLD

    elif side == "short":
        mask[1] = True   # SELL 1x
        mask[3] = True   # SELL 1.8x
        mask[4] = True   # HOLD
...
def build_action_mask_from_positions(positions: torch.Tensor, side: str) -> torch.Tensor:
    """
    positions : (B,) avec valeurs -1, 0, +1
    Retourne un mask (B, N_ACTIONS) bool :
      - si flat (0)  : dépend de side ("both"/"long"/"short")
      - si en pos    : seule HOLD (index 4) valide
    """
    ...
    if flat.any():
        if side == "both":
            ...
            mask[flat, 0] = True   # BUY1
            mask[flat, 2] = True   # BUY1.8
            mask[flat, 4] = True   # HOLD

        elif side == "short":
            mask[flat, 1] = True   # SELL1
            mask[flat, 3] = True   # SELL1.8
            mask[flat, 4] = True   # HOLD

        else:
            mask[flat] = True

    # Si DÉJÀ EN POSITION → seulement HOLD
    if inpos.any():
        mask[inpos, 4] = True
...
def run_training(cfg: PPOConfig):
    ...
    # chemins pour les modèles (dépendent de side / model_prefix)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        batch_returns = []
        batch_values = []
        batch_positions = []  # position à chaque step
        ...
        for ep in range(cfg.episodes_per_epoch):
            ...
            while not done:
                ...
                # on stocke la position AVANT action
                ...
                with torch.no_grad():
                    ...
                    if pos == 0:  # flat → toutes actions autorisées par le masque
            ...
            # GAE sur reward (log-returns Sharpe-like)
            adv, ret = compute_gae(
        ...
        assert states.size(0) == oldlog.size(0) == actions.size(0) == values_old.size(0) == positions.size(0), \
            f"Len mismatch: states={states.size(0)}, oldlog={oldlog.size(0)}, actions={actions.size(0)}, values={values_old.size(0)}, positions={positions.size(0)}"

        advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-8)
        advantages = advantages.clamp(-10, 10)
        advantages = advantages * 1.5  # boost modéré du signal
    ...
    print(f"[{cfg.side.upper()}] Entraînement terminé, passage en TEST (epsilon-greedy + mask)…")
...
if __name__ == "__main__":
    # Exemple : lancer UN agent à la fois.
    # Pour un agent BUY-only :
    #   cfg = PPOConfig(side="long", model_prefix="saintv2_loup")
    # Pour un agent SELL-only :
    #   cfg = PPOConfig(side="short", model_prefix="saintv2_loup")
    # Pour l’agent symétrique (long+short) comme avant :
    #   cfg = PPOConfig(side="both", model_prefix="saintv2_loup")

    # Ici, tu choisis celui que tu veux entraîner :
    cfg = PPOConfig(side="long", model_prefix="saintv2_loup_long")
    run_training(cfg)

    # Si tu veux enchaîner short juste après, tu peux décommenter :
